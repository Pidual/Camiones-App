...
export const FarmerJobsScreen = ({ navigation }) => {
  ...
  const loadJobs = async () => {
    try {
      const res = await jobsAPI.getMyJobs('farmer');
      setJobs(res.data);
    } catch (error) {
  ...
  const renderJob = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('FarmerJobDetail', { job: item })}
    >
      ...
      <View style={styles.jobFooter}>
        ...
        <Text style={styles.requests}>{item.requests?.length || 0} requests</Text>
      </View>
    </TouchableOpacity>
  );
...
const styles = StyleSheet.create({
  ...
  payment: { fontSize: 16, fontWeight: 'bold', color: '#4CAF50' },
  requests: { fontSize: 12, color: '#999' },
